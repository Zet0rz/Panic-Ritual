...
function EFFECT:Init(data)
	local t = data:GetFlags()
	if t > 0 and t ~= LocalPlayer():Team() then self.KillTime = 0 self:Remove() return false end -- kill immediately
...
local particlethink = function(p)
	--if true then print(p:GetAngles():Forward()) p:SetNextThink(CurTime() + updaterate) return end
